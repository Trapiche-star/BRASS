...
namespace BRASS
{
    ...
    public class PlayerInputHandler : MonoBehaviour
    {
        ...
        public bool ClickMovePressed { get; private set; } // 마우스 좌클릭 이동 입력
        ...
        public bool IsKeyboardMove { get; private set; }   // WASD 수동 이동 여부
        #endregion
        ...
        {
            if (playerInput == null)
                playerInput = GetComponent<PlayerInput>(); // 만약 [참조가 없으면] [컴포넌트를 자동 탐색한다]
        }

        private void OnEnable()
        {
            if (playerInput == null) return; // 만약 [참조가 없으면] [입력 이벤트를 구독하지 않는다]
        ...
        private void Update()
        {
            if (Mouse.current != null)
                MousePosition = Mouse.current.position.ReadValue(); // 만약 [마우스가 있으면] [현재 마우스 좌표를 갱신한다]
        }
        #endregion
        ...
        {
            ZoomInput = 0f; // 만약 [줌 입력을 사용했으면] [다음 프레임에 누적되지 않게 초기화한다]
        }

        // WASD 이동 입력 수집
        private void OnMove(InputAction.CallbackContext context)
        {
            MoveInput = context.ReadValue<Vector2>(); // 이동 입력값 갱신
            IsKeyboardMove = MoveInput.sqrMagnitude > 0.01f; // 만약 [입력이 존재하면] [수동 이동으로 판단한다]
        }

        // 좌클릭 이동 입력 수집
        private void OnClickMove(InputAction.CallbackContext context)
        {
            ClickMovePressed = context.ReadValueAsButton(); // 만약 [버튼이 눌리면] [클릭 이동으로 판단한다]
        }
